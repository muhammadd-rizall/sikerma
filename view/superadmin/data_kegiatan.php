...
?>
    ...
    ?>
        ...
        ?>
         ...
         <h2>Edit Kegiatan</h2>
             <form action="proses_data_mou_moa.php?aksi=edit&id_edit=<?= $dataKegiatan['id_kegiatan'] ?>" method="post" enctype="multipart/form-data">
                 <div class="form-group">
                     <label for="nama">Kegiatan</label>
                 ...
                 <div class="form-group"> </div>```php
$aksi = isset($_POST['aksi']) ? $_POST['aksi'] : (isset($_GET['aksi']) ? $_GET['aksi'] : 'list');
switch ($aksi) :
    case "list":  
        ?>
        <h2>Tabel Kegiatan</h2>
        <a href="?aksi=input" class="btn btn-primary mb-2">Tambah Kegiatan</a>
        <table id="tabel-user" class="table table-bordered table-striped">
            <thead>
                <tr>
                    <th>No</th>
            ...
            <tbody>
                ...
                ?>
                    <tr>
                        ...
                        <td><?= $dataKegiatan['dokumentasi'] ?></td>
                        <td>
                            <a href="?aksi=edit&id_edit=<?= $dataKegiatan['id_kegiatan'] ?>" class="btn btn-warning">Edit</a>
                            <a href="proses_data_mou_moa.php?proses=delete&id_hapus=<?= $dataKegiatan['id_kegiatan'] ?>" class="btn btn-danger" onclick="return confirm('Yakin akan menghapus data?')">Hapus</a>
                        </td>
                    </tr>
                <?php
                $no++;
    ...
    case "edit":
        $id_edit = $_GET['id_edit'];
        $ambil = mysqli_query($koneksi, "SELECT * FROM tb_kegiatan_kerjasama WHERE id_kegiatan = '$id_edit'");
        $dataKegiatan = mysqli_fetch_array($ambil);
    ?>
        <h2>Edit Kegiatan</h2>
        <form action="proses_data_mou_moa.php?aksi=edit&id_edit=<?= $dataKegiatan['id_kegiatan'] ?>" method="post" enctype="multipart/form-data">
